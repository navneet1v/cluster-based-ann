 ...
 *
 * <p>This class leverages the Java Vector API (JDK 23+) to perform vectorized operations on float
 * arrays, utilizing Single Instruction Multiple Data (SIMD) instructions when available on the
 * target hardware. Operations automatically fall back to scalar computation for array elements that
 * don't fit in complete SIMD vectors.
 *
 * <p>All methods in this class are static and thread-safe. The class uses the preferred vector
 * species for the target platform to maximize performance.
 *
 * <h3>Performance Considerations:</h3>
 *
 * <ul>
 *   <li>SIMD operations provide significant speedup for large arrays
 *   <li>Performance gains are most noticeable with arrays larger than the SIMD register size
 *   <li>Optimal performance requires proper JVM flags: {@code --add-modules jdk.incubator.vector}
 * </ul>
 *
 * <h3>Usage Example:</h3>
 *
...
public class VectorDistanceCalculationUtils {
     ...
     *
     * <p>The squared Euclidean distance is calculated as the sum of squared differences: distance²
     * = Σ(a[i] - b[i])²
     *
     * <p>This implementation uses the Vector API for SIMD acceleration, processing multiple
     * elements simultaneously when possible, and falls back to scalar computation for remaining
     * elements.
     *
     ...
     *
     * <p>This overload is optimized for off-heap memory access, avoiding array allocation and
     * copying overhead. It directly reads from MemorySegments using the Vector API.
     *
     ...
     *
     * <ul>
     *   <li>5-15% faster than array-based version for off-heap data
     *   <li>No intermediate array allocation
     *   <li>Direct memory access with SIMD operations
     *   <li>Better cache locality for sequential access
     * </ul>
     *
     * <p><b>Implementation Details:</b>
     *
     * <ul>
     *   <li>SIMD loop processes vectorLength floats per iteration (8 for AVX2, 16 for AVX-512)
     *   <li>Remainder loop handles elements that don't fit in complete SIMD vectors
     *   <li>Uses native byte order for optimal performance
     * </ul>
     *
    ...
    public static float euclideanDistance(
            final MemorySegment a, final MemorySegment b, int length) {
        int vectorLength = SPECIES.length();
        ...
        for (long i = 0; i < loopBound; i += vectorLength) {
            FloatVector va =
                    FloatVector.fromMemorySegment(
                            SPECIES, a, i * Float.BYTES, ByteOrder.nativeOrder());
            FloatVector vb =
                    FloatVector.fromMemorySegment(
                            SPECIES, b, i * Float.BYTES, ByteOrder.nativeOrder());
    ...
    /**
     * Computes the squared Euclidean distance between a MemorySegment and a float array using SIMD
     * optimization.
     *
     ...
     *
     * <ul>
     *   <li>Searching with query vector (array) against indexed vectors (MemorySegment)
     *   <li>Mixed storage scenarios (off-heap index, on-heap queries)
     *   <li>Avoiding unnecessary array-to-segment conversions
     * </ul>
     *
     * <p><b>Performance:</b>
     *
     * <ul>
     *   <li>Faster than converting array to MemorySegment first
     *   <li>SIMD acceleration for both data sources
     *   <li>Optimal when 'a' is frequently reused (cached in off-heap storage)
     * </ul>
     *
     * @param a the first vector as a MemorySegment (off-heap memory)
     ...
     *
     * <p>The inner product is calculated as the sum of element-wise products: result = Σ(a[i] *
     * b[i])
     *
     * <p>This implementation uses the Vector API for SIMD acceleration, processing multiple
     * elements simultaneously when possible, and falls back to scalar computation for remaining
