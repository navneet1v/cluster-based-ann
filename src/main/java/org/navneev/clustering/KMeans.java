...
/**
 * K-means clustering implementation for vector data stored in VectorStorage.
...
public class KMeans {
    ...
    public int[] fit(final IntegerList sampledVectorIds, final VectorStorage vectors) {
        ...
        for (int iter = 0; iter < maxIterations; iter++) {
            int[] newLabels = assignClusters(sampledVectorIds, vectors); // Assign points to nearest centroid
    ...
    private int[] assignClusters(IntegerList sampleVectorIds, VectorStorage vectors) {
        ...
        for (int i = 0; i < sampleVectorIds.size(); i++) {
            ...
            for (int j = 0; j < k; j++) {
                ...
                float dist = VectorDistanceCalculationUtils.euclideanDistance(vectors.getMemorySegment(vectorDocId), centroids[j]); // Calculate
                // distance
     ...
     */
    private void updateCentroids(IntegerList sampledVectorIds, VectorStorage vectors, int[] labels) {
